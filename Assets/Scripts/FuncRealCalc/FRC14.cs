...
public class FRC14 : MonoBehaviour
{
    //calcメソッドを提議
    ...
    void funcNumberIn()
    {
        for (float i = -5f * Mathf.Sqrt(3); i <= 5f * Mathf.Sqrt(3); i = i + GameManager.Instance.rsl)
        {
            for (float j = -5f * Mathf.Sqrt(3); j <= 5f * Mathf.Sqrt(3); j = j + GameManager.Instance.rsl)
            {
                float calcReturn = calc(i, j);
                if (Mathf.Abs(calcReturn) <= GameManager.Instance.rsl)
                {
                    GameManager.Instance.graphPointsX.Add(i * Mathf.Sqrt(3) / 2 - j / 2 - 2f);
                    GameManager.Instance.graphPointsY.Add(j * Mathf.Sqrt(3) / 2 + i / 2 + 2f);
                }
            }
        }
    }
