...
public class FRC6 : MonoBehaviour
{
    //calcメソッドを提議
    ...
    void funcNumberIn()
    {
        for (float i = 0f; i <= 6f; i = i + GameManager.Instance.rsl)
        {
            for (float j = 0f; j <= 5f; j = j + GameManager.Instance.rsl)
            {
                float calcReturn = calc(i, j);
                if (Mathf.Abs(calcReturn) <= GameManager.Instance.rsl)
                {
                    GameManager.Instance.graphPointsX.Add(i - 3f);
                    GameManager.Instance.graphPointsY.Add(j);
                }
            }
        }
    }
