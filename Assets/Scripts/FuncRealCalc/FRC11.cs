...
public class FRC11 : MonoBehaviour
{
    //calcメソッドを提議
    ...
    void funcNumberIn()
    {
        for (float i = -3f * Mathf.Sqrt(2); i <= 3f * Mathf.Sqrt(2); i = i + GameManager.Instance.rsl)
        {
            for (float j = -2.5f * Mathf.Sqrt(2); j <= 2.5f * Mathf.Sqrt(2); j = j + GameManager.Instance.rsl)
            {
                float calcReturn = calc(i, j);
                if (Mathf.Abs(calcReturn) <= GameManager.Instance.rsl)
                {
                    GameManager.Instance.graphPointsX.Add(i / Mathf.Sqrt(2) - j / Mathf.Sqrt(2));
                    GameManager.Instance.graphPointsY.Add(j / Mathf.Sqrt(2) + i / Mathf.Sqrt(2) + 2.5f);
                }
            }
        }
    }
