...
public class FRC12 : MonoBehaviour
{
    //calcメソッドを提議
    ...
    void funcNumberIn()
    {
        ...
        {
            for (float j = 0f * Mathf.Sqrt(2); j <= 5f * Mathf.Sqrt(2); j = j + GameManager.Instance.rsl)
            {
                float calcReturn = calc(i, j);
                if (Mathf.Abs(calcReturn) <= GameManager.Instance.rsl)
                {
                    GameManager.Instance.graphPointsX.Add(i / Mathf.Sqrt(2) - j / Mathf.Sqrt(2) + 3f);
                    GameManager.Instance.graphPointsY.Add(j / Mathf.Sqrt(2) + i / Mathf.Sqrt(2));
                }
            }
        }
    }
